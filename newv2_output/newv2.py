...
def model1(input_shape):
    ...
    #c7 = Conv2D(32, (3, 3), activation='relu', padding='same') (c3)
   # c7 = Conv2D(32, (3, 3), activation='relu', padding='same') (c7)

    #u8 = Conv2DTranspose(16, (2, 2), strides=(2, 2), padding='same') (c7)
    #u8 = concatenate([u8, c2])
    c8 = Conv2D(16, (3, 3), activation='relu', padding='same') (c2)
    #c8 = Conv2D(16, (3, 3), activation='relu', padding='same') (c8)

    #u9 = Conv2DTranspose(8, (2, 2), strides=(2, 2), padding='same') (c8)
    #u9 = concatenate([u9, c1], axis=3)
    c9 = Conv2D(8, (3, 3), activation='relu', padding='same') (c8)
...
def r2_score(y_true, y_pred):
    total_error = np.sum(np.square(y_true - np.mean(y_true)))
    residual_error = np.sum(np.square( y_true - y_pred ))
    R_squared = 1 - (residual_error / total_error)
    return -R_squared
